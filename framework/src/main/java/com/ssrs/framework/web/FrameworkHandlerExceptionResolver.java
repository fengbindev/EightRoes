...
public class FrameworkHandlerExceptionResolver extends AbstractHandlerExceptionResolver {
    ...
    protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler,
        ...
        if (response.getStatus() < HttpServletResponse.SC_INTERNAL_SERVER_ERROR) {
            log.info(ex, "Info: doResolveInfo {}", ex.getMessage());
        } else {
            log.warn(ex, "Warn: doResolveException {}", ex.getMessage());
        }
     ...
     *
     * @param ex       the AuthorizationException to be handled
     * @param request  current HTTP request
     * @param response current HTTP response
     */
    protected void handleAuthorizationException(AuthorizationException ex,
                                                HttpServletRequest request, HttpServletResponse response) {
        Throwable cause = ex.getCause();
        if (ex.getCause() != null && cause instanceof AuthorizationException) {
            ResponseUtils.sendFail(request, response, ErrorCodeEnum.FORBIDDEN, (AuthorizationException) cause);
            return;
        }
